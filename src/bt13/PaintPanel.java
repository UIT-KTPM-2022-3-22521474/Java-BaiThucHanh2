...
public class PaintPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void floodFill(Point2D.Double point, Color fillColor) {
		...
		while (!queue.isEmpty()) {
			...
			if ((int) p.getX() >= 0 && (int) p.getX() < canvas.getWidth() && (int) p.getY() >= 0
					&& (int) p.getY() < canvas.getHeight())
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (activeTool == LINE_TOOL && dragged) {
			...
			if (i == TextDialog.APPLY_OPTION) {
				shapes.push(new Shape(x1, y1, td.getInputSize(), td.getFont(), primary, stroke, 5, td.getText())); // int
																													// x1,
																													// int
																													// y1,
																													// int
